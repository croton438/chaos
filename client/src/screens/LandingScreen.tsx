      ...
      <section className="relative z-10 w-full max-w-md">
        ...
        <div className="glass-panel rounded-3xl p-7 sm:p-9">
          <div className="mb-7 text-center">
            <p className="mb-3 text-xs font-bold uppercase tracking-[0.35em] text-chaos-violet">{t("landing.tagline")}</p>
            <h1 className="text-3xl font-black tracking-tight text-white">{t("landing.title")}</h1>
            <p className="mt-3 text-sm leading-6 text-zinc-500">{t("landing.description")}</p>
          </div>
          <form onSubmit={submit} className="space-y-4">
            <label className="block">
              <span className="mb-2 block text-xs font-bold uppercase tracking-widest text-zinc-500">{t("landing.username")}</span>
            ...
            <Button className="w-full" disabled={!username.trim()} type="submit">{t("landing.enter")} <ArrowRight size={18} /></Button>
          </form>
          <div className="mt-7 grid grid-cols-2 gap-3 border-t border-white/5 pt-6 text-xs text-zinc-500">
            <span className="flex items-center gap-2"><Users size={15} className="text-chaos-violet" /> {t("landing.rooms")}</span>
            <span className="flex items-center gap-2"><Waves size={15} className="text-chaos-cyan" /> {t("landing.voice")}</span>
          </div>
        </div>
      </section>
    </main>
